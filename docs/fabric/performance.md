...
| createPrivateAsset | Performs a single `putPrivateData()` operation, inserting an asset of defined byte size into the Private Data collection of the authorized organization. |
...
| getPrivateAsset | Performs a single `getPrivateData()` operation, extracting and returning a single asset from the Private Data Collection of the authorized organization using a passed UUID. |
